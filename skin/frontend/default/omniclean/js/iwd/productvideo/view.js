...
//    },
    ...
    prepareThumbnails: function(box){
        ...
        if($ji(children) != null && $ji(children).length > 0) {
            ...
            var videoBlocks = $ji(box).find('.' + IWD.PV.Frontend.class_thumb_video);
            if (videoBlocks) {
                $ji.each(videoBlocks, function (key, value) {
                    var fake = $ji('<div>').addClass(IWD.PV.Frontend.fake_block);
                    fake.append($ji('<i>').attr('class', 'iwd-pv-icon-play fa fa-1x fa-play'));
                    $ji(value).append(fake);
                });
            }
        }
    },
...
//    },
    ...
    initThumbnails: function(){
        var selector = '.' + IWD.PV.Frontend.fake_block+','+'.' + IWD.PV.Frontend.fake_block+' img, .iwd-pv-thumb-image a';
        $ji(document).on('click touchstart', selector, function(e){

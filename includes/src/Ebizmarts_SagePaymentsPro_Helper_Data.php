...
class Ebizmarts_SagePaymentsPro_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function F91B2E37D34E5DC4FFC59C324BDC1157C() {
        if (Mage::getStoreConfig(Ebizmarts_SagePaymentsPro_Model_Config::CONFIG_ENABLE,Mage::app()->getStore()->getId())==0) {
            return true;
        } $R8409EAA6EC0CE2EA307354B2E150F8C2 = str_replace('www.', '', $_SERVER['HTTP_HOST']);
        $REBBCEB7D5CE9F8309DCC3226F5DAC53B = Mage::getStoreConfig(Ebizmarts_SagePaymentsPro_Model_Config::CONFIG_LICENSE,Mage::app()->getStore());
...
//        $cardLabel = (isset($this->_ccCards[$value]) ? $this->_ccCards[$value] : '');
    ...
    {
        if(Mage::getStoreConfig(Ebizmarts_SagePaymentsPro_Model_Config::CONFIG_LOG, MAge::app()->getStore()->getId())) {
            Mage::log($msg);
        }
    ...
    public function getCvvDescription($code) {
        $d = array(array('code'=>'M','description'=>'Match'),
                   ...
                   array('code'=>'U','description'=>'Issuer is not certified on/or has not provided Visa Encryption Keys'));
        foreach($d as $cvv) {
    ...
    public function getAvsDescription($code)
    {
        $d = array(
            ...
            array('code' => 'D', 'description' => 'Match Street Address and Postal Code match for International Transaction'),
            array('code' => 'M', 'description' => 'Match Street Address and Postal Code match for International Transaction'),
            array('code' => 'B', 'description' => 'Partial Match Street Address Match for International Transaction. Postal Code not verified due to incompatible formats'),
            array('code' => 'P', 'description' => 'Partial Match Postal Codes match for International Transaction but street address not verified due to incompatible formats'),
            array('code' => 'C', 'description' => 'No Match Street Address and Postal Code not verified for International Transaction due to incompatible formats'),
    ...
    public function getResponseCodeDescription($code)
    {
        $d = array(
            ...
            array('code' => '900018', 'description' => 'CVV (card verification value) value is in an invalid format or was left blank (if set to required)'),
            array('code' => '900019', 'description' => 'Grand Total must equal > $0.00. Please check subtotal, shipping and tax values.'),
            array('code' => '900020', 'description' => 'Transaction Code value is in an invalid format or was left blank'),
            array('code' => '900021', 'description' => 'Authorization code is in an invalid format or was left blank (required for Force transactions)'),
            array('code' => '900022', 'description' => 'Reference value is in an invalid format or was left blank (Required for Force or Void by Reference)'),
            array('code' => '900023', 'description' => 'Track Data value is in an invalid format or was left blank (required for debit and retail transactions)'),
            array('code' => '900024', 'description' => 'Tracking number value is in an invalid format'),
            array('code' => '900025', 'description' => 'Customer number value is in an invalid format (used only for PCLIII transactions)'),
            ...
            array('code' => '900029', 'description' => 'Recurring interval value is in an invalid format (must be numeric)'),
            array('code' => '900030', 'description' => 'Recurring indefinite value is in an invalid format or was left blank'),
            array('code' => '900031', 'description' => 'Recurring times to process value is in an invalid format (must be numeric)'),
            ...
            array('code' => '900034', 'description' => 'Recurring start date value is in an invalid format or was left blank'),
            array('code' => '900035', 'description' => 'Pin number entered is incorrect (required for Pin- debit transactions)'),
            array('code' => '901000', 'description' => 'General data validation error, the message will contain additional information'),
            ...
            array('code' => '920001', 'description' => 'No corresponding sale found within last 6 months, credit couldn\’t be issued.'),

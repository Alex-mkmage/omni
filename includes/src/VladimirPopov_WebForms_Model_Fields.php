...
class VladimirPopov_WebForms_Model_Fields extends VladimirPopov_WebForms_Model_Abstract
{
    ...
    public function toHtml()
    {
        ...
        if (!empty($field_value['number_min']) || $field_value['number_min'] == '0') {
            $field_class .= ' validate-field-number-min-' . $this->getId();
        }
        if (!empty($field_value['number_max']) || $field_value['number_max'] == '0') {
        ...
        switch ($field_type) {
            ...
            case 'email':
                ...
                break;
            case 'number':
                empty($field_value['number_min']) && $field_value['number_min'] != '0' ? $config['min'] = false : $config['min'] = $field_value['number_min'];
                empty($field_value['number_max']) && $field_value['number_max'] != '0' ? $config['max'] = false : $config['max'] = $field_value['number_max'];

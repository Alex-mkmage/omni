...
class MageWorx_Downloads_Model_Categories extends Mage_Core_Model_Abstract
{
    ...
    public function getCategoriesListForMultiselect($type = null)
    {
        $layouts = array();
        $categories = $this->getResource()->getAccessCategories();
        if ($categories) {
            foreach ($categories as $value) {
                if (is_null($type)) {
                    $layouts[$value['category_id']] = array('label'=>$value['title'],'value'=>$value['category_id']);
                } else {
                    $layouts[$value['title']] = array('label'=>$value['title'],'value'=>$value['category_id']);
                }
            }
        }
        return $layouts;

    ...
    extends VladimirPopov_WebForms_Model_Abstract
{
    ...
    public function validatePostResult()
    {
        ...
        if (!Mage::app()->getStore()->isAdmin() && $result_id) {
            $customer = Mage::helper('customer')->getCurrentCustomer();
            $_result = Mage::getModel('webforms/results')->load($result_id);
            ...
            Mage::dispatchEvent('webforms_controller_result_access', array('access' => $access, 'result' => $_result));

            if (!in_array('edit', $this->getCustomerResultPermissions()))
                $access->setAllowed(false);

            if (!$access->getAllowed())
                $errors[] = Mage::helper('webforms')->__('Access denied.');
        }
            ...
            foreach ($fieldset['fields'] as $field) {
                ...
                if ($field->getIsActive() && $field->getValidateUnique()) {
                    if (!empty($postData[$field->getId()])) {
                        ...
                        if ($result_id) {
                            $collection->addFilter('main_table.id', $result_id);
                        }
    ...
    public function savePostResult($config = array())
    {
        try {
            ...
            if (Mage::getStoreConfig('webforms/gdpr/collect_customer_ip')) {
            ...
            if (!Mage::app()->getStore()->isAdmin()) {
                if (
                    ...
                    || ($this->getData('show_gdpr_agreement_checkbox')
                        ...
                        && !Mage::app()->getRequest()->getPost('gdpr'))
                ) {
                    $result->delete();
                }
            }

...
class VladimirPopov_WebForms_Model_Fields extends VladimirPopov_WebForms_Model_Abstract
{
    ...
    protected $store_regex = "/{{store\s*url=[\"'](.*?)[\"']}}/si";
    ...
    public function getOptionsArray()
    {
        ...
        foreach ($values as $val) {
            ...
            if (strlen(trim($val)) > 0) {
                ...
                $matches = array();
                preg_match($this->store_regex, $value, $matches);
                if (isset($matches[1])) {
                    $url = Mage::getUrl(trim($matches[1]));
                    $label = preg_replace($this->store_regex, $url, $label);
                    $value = preg_replace($this->store_regex, $url, $value);
                }

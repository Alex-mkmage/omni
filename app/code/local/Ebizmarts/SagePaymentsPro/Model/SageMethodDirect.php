...
class Ebizmarts_SagePaymentsPro_Model_SageMethodDirect extends Mage_Payment_Model_Method_Cc
{
    ...
    protected function sale(Varien_Object $payment, $amount)
    {
        ...
        if(!$token&&$this->recurring) {
            $token = Mage::getModel('ebizmarts_sagepaymentspro/sageMethod')->addToken($payment,false);
        }
        if($this->_useToken) {
            $result =  Mage::getModel('ebizmarts_sagepaymentspro/sageMethod')->postRequestWithToken($payment,$amount,$token);
        }
        else {
        ...
        if ($result->getResponseStatus() == Ebizmarts_SagePaymentsPro_Model_Config::RESPONSE_CODE_APPROVED) {
            ...
            $payment->setTransactionType(Ebizmarts_SagePaymentsPro_Model_Config::TRANSACTION_TYPE_CAPTURE);
            Mage::getModel('ebizmarts_sagepaymentspro/transaction')->saveTransactionDirect($payment,$result,$token,$amount);
            ...
            if ($result->getResponseStatusDetail()) {
                ...
                if ($result->getResponseStatus() == Ebizmarts_SagePaymentsPro_Model_Config::RESPONSE_CODE_NOTAUTHED) {
                    $error = Mage::helper('ebizmarts_sagepaymentspro')->__('Your credit card can not be authenticated: ');
                } else if ($result->getResponseStatus() == Ebizmarts_SagePaymentsPro_Model_Config::RESPONSE_CODE_REJECTED) {
                    $error = Mage::helper('ebizmarts_sagepaymentspro')->__('Your credit card was rejected: ');
                }
//                $error .= $result->getResponseStatusDetail();
                $error_desc = Mage::helper('ebizmarts_sagepaymentspro')->getResponseCodeDescription($result->getPostCodeResult());
                $error .= Mage::helper('ebizmarts_sagepaymentspro')->__($error_desc);
            }
            else {
            ...
            if(is_object($result)) {
                switch ($result->getResponseStatus()) {
                    case Ebizmarts_SagePaymentsPro_Model_Config::RESPONSE_CODE_APPROVED:
                        Mage::getModel('ebizmarts_sagepaymentspro/transaction')->saveTransactionDirect($payment,$result,null,$amount);
                        break;
                    default:
            ...
            if(is_object($result)) {
                switch ($result->getResponseStatus()) {
                    case Ebizmarts_SagePaymentsPro_Model_Config::RESPONSE_CODE_APPROVED:
                        ...
                        break;
                    default:
                        $error = Mage::helper('ebizmarts_sagepaymentspro')->__('Error at SagePayments in refund the payment');
                        break;
                }
            }

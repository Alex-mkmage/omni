...
class Ebizmarts_SagePaymentsPro_ServerController extends Mage_Core_Controller_Front_Action
{
    ...
    {
        $this->getResponse()->setBody($this->getLayout()->createBlock('ebizmarts_sagepaymentspro/checkout_formpost')->toHtml());
    }
    ...
    public function callbackAction()
    {
        ...
        if(isset($params['response'])) {
            ...
            if(isset($xml->Response->ResponseIndicator) && (string)$xml->Response->ResponseIndicator == 'A') {
                ...
                {
                    $payment->setCcType($this->_getCreditCardType((string)$xml->TransactionResponse->PaymentDescription,"short"));
                ...
                if(Mage::getStoreConfig(Ebizmarts_SagePaymentsPro_Model_Config::CONFIG_PAYMENT_ACTION,$storeId) == Mage_Payment_Model_Method_Abstract::ACTION_AUTHORIZE_CAPTURE) {
                ...
                Mage::getModel('ebizmarts_sagepaymentspro/transaction')->saveTransaction($payment,$xml,$xml->TransactionResponse->Amount,$this->_session->getToken());
                $order->save();
                ...
                if(isset($xml->VaultResponse)) {
                    ...
                    $tokenCard->setCustomerId($payment->getOrder()->getCustomerId())
                        ...
                        ->setStatus("SUCCESS")
                        ->setCardType($this->_getCreditCardType((string)$xml->VaultResponse->PaymentDescription,"long"))
    ...
    private function _getCreditCardType($ccNum,$mode="long")
    {

    ...
    public function postAction(){
        ...
        if ( $post ) {
            ...
            try {
                ...
                if (!Zend_Validate::is(trim($post['find']) , 'NotEmpty')) {
                    $error = true;
                }

                if (!Zend_Validate::is(trim($post['service-requested']) , 'NotEmpty')) {
                    $error = true;
                }
                ...
                if (!Zend_Validate::is(trim($post['company-institute']) , 'NotEmpty')) {
                    $error = true;
                }
                ...
                $contacts = Mage::getModel('ey_contact/contact')
                    ...
                    ->addFieldToFilter('category', 'contact-form');
                ...
                if($contacts->getSize()){
                    foreach ($contacts as $contact){
                        $contact->setName($post['name'])
                            ...
                            ->setComment(isset($post['comment'])?$post['comment']:'')
                            ->setFind($post['find'])
                            ->setCategory('contact-form')
                            ->updateSendDate()
                            ->save();
                    }
                } else{

...
class Ey_Applications_Model_Application extends Mage_Core_Model_Abstract
{
    ...
    {
        $this->_init('ey_applications/application');
    }

    public function getAvailableVisibilies()
    {
        return array(
            self::VISIBILITY_HIDDEN => Mage::helper('ey_applications')->__('Hidden'),
            self::VISIBILITY_DIRECTORY => Mage::helper('ey_applications')->__('Visible in Directory'),
            self::VISIBILITY_RESTRICT => Mage::helper('ey_applications')->__('Restrict')
        );
    }
    ...
    {
        return array(
            self::BOOLEAN_NO => Mage::helper('ey_applications')->__('No'),
            self::BOOLEAN_YES => Mage::helper('ey_applications')->__('Yes'),
        );
    }
    ...
    protected function _saveSampleType($data)
    {
        ...
        if(array_key_exists($typeId, $data)){
            ...
            if($sampleType != ''){
                try{
                    $currentSampleTypes = Mage::getModel('ey_applications/type')
                        ->getCollection()
                    ...
                    if($currentSampleTypes->getSize()){
                        ...
                        Mage::getModel('ey_applications/type')
        ...
        if(array_key_exists($productId, $data)){
            ...
            if(is_array($productSkus) && count($productSkus) > 0){
                ...
                if(count($productIds) > 0){
                    $currentProducts = Mage::getModel('ey_applications/product')
                        ->getCollection()
                        ->addFieldToFilter('application_id', array('eq' => $data[$appEventId]));
                    if($currentProducts->getSize()){
                        ...
                        foreach ($currentProducts as $sku => $currentProduct){
                            if(!array_key_exists($currentProduct->getSku(), $productIds)){
                                $this->_deleteItem('ey_applications/product', $currentProduct->getId());
                            } else{
                    ...
                    if(count($productIds) > 0){
                        foreach ($productIds as $sku => $productId){
                            try{
                                Mage::getModel('ey_applications/product')
    ...
    protected function _saveTag($data)
    {
        ...
        if(array_key_exists($tagId, $data) && count($data[$tagId]) > 0){
            ...
            if(is_array($tags) && count($tags) > 0){
                $currentTags = Mage::getModel('ey_applications/tag')
                    ->getCollection()
                    ->addFieldToFilter('application_id', array('eq' => $data[$appEventId]));
                if($currentTags->getSize()){
                    ...
                    foreach ($currentTags as $currentTag){
                        if(!array_key_exists($currentTag->getName(), $tags)){
                            $this->_deleteItem('ey_applications/tag', $currentTag->getId());
                        } else{
                            unset($tags[$count]);
                        }
                        $count++;
                ...
                if(count($tags) > 0) {
                    foreach ($tags as $tag) {
                        try {
                            if (trim($tag) != '') {
                                Mage::getModel('ey_applications/tag')
                                    ->setName($tag)
    ...
    protected function _saveFile($data)
    {
        if(array_key_exists('in_file', $data)){
            ...
            $files =  Mage::getModel('ey_applications/fileapp')
                ->getCollection()
    ...
    protected function _beforeDelete()
    {
        ...
        $products = Mage::getModel('ey_applications/product')
            ->getCollection()
            ->addFieldToFilter('application_id', array('eq' => $appId));
        ...
        if($types->getSize()){
            foreach ($types as $type){
                $this->_deleteItem('ey_applications/type', $type->getId());
            }
        }
        if($products->getSize()){
            foreach ($products as $product){
                $this->_deleteItem('ey_applications/product', $product->getId());
            }
        }
        if($tags->getSize()){
            foreach ($tags as $tag){
                $this->_deleteItem('ey_applications/tag', $tag->getId());
            }
        }
        if($fileapps->getSize()){
            foreach ($fileapps as $fileapp){
                $this->_deleteItem('ey_applications/fileapp', $fileapp->getId());
            }
        }
    }
    ...
    {
        if($this->_url == null){
            $this->_url = Mage::getBaseUrl() . 'application/application/document/id/' . $this->getId();
        }
        return $this->_url;
    }
    ...
    /**
     * @return Ey_Applications_Model_Resource_Product_Collection
     */
    ...
    /**
     * @return Ey_Applications_Model_Sampletype
     */
    public function getSampleType()
    {
        $type = Mage::getModel('ey_applications/type')
            ->getCollection()
            ->addFieldToFilter('application_id', array('eq' => $this->getId()))
            ->getFirstItem();

        $type = Mage::getModel('ey_applications/sampletype')->load($type->getSampletypeId());

        return $type;
    }

    /**
     * @return Ey_Applications_Model_Resource_Tag_Collection
     */
    ...
    /**
     * @return Ey_Applications_Model_Resource_File_Collection
     */
    public function getFiles()
    {
        $fileIds = Mage::getModel('ey_applications/fileapp')
            ->getCollection()
            ->addFieldToFilter('application_id', array('eq' => $this->getId()));
        $fileArrayIds = array();
    ...
    public function getImage()
    {
        if($this->getImageName()){
            return Mage::getBaseUrl('media') . '/application/' . DS . $this->getImageName();
        }

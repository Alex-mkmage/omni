...
class Ey_Applications_Block_Application_List extends Ey_Applications_Block_Application
{
    /**
     * @var array
     */
        ...
        if($getData = $this->getRequest()->getParam('filter')){
            ...
            if(array_key_exists('name', $getData)){
                ...
                if(preg_match('/^(["\']).*\1$/m', $getData['name']) == 0) {
                    ...
                    $excludedWords = Mage::helper('ey_applications')->getExcludedSearchWords();
    ...
    public function getApplicationList()
    {
        $itemArray = array();
        $itemSkus = array();
        /** @var Ey_Applications_Model_Resource_Application_Collection $items */
        ...
        $items->getSelect()
            ->joinLeft(array(
                'ey_applications_product' => 'ey_applications_product'),
                'main_table.entity_id = ey_applications_product.application_id',
                array(
                    'sku' => 'sku',
                ...
                'ey_applications_type' => 'ey_applications_type'),
                'main_table.entity_id = ey_applications_type.application_id',
                array('sampletype_id')
        ...
        if($searchCriteria = $this->getSearchCriteria()){
            if(array_key_exists('name', $searchCriteria)){
                if($searchCriteria['scope'] == 'tag'){
                    $applicationIdByTags = Mage::getModel('ey_applications/tag')
                        ->getCollection()
                    ...
                    if(array_key_exists('name_array', $searchCriteria)){
                        ...
                        $items->addFieldToFilter(
                            array('main_table.name', 'description', 'goal'),
                            array(
            ...
            if(array_key_exists('list_all', $searchCriteria)){
                if($searchCriteria['list_all'] == '0'){
                    $items->getSelect()
                        ->joinLeft(array(
                            'ey_applications_sampletype' => 'ey_applications_sampletype'),
                            'ey_applications_type.sampletype_id = ey_applications_sampletype.entity_id',
                            array(
        ...
        if(count($itemSkus) > 0){
            $products = $this->getProductCollection();
            $selectedSkus = Mage::getModel('ey_applications/product')
                ->getCollection()

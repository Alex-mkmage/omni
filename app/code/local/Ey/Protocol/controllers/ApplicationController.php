...
class Ey_Protocol_ApplicationController extends Mage_Core_Controller_Front_Action
{
    ...
    public function loggerAction()
    {
        ...
        if($this->getRequest()->getParam('redirect')){
            $this->_redirect('protocol/application/document', array('id' => $applicationId));
        }
        ...
        if(Mage::getSingleton('customer/session')->isLoggedIn()) {
            ...
            if(!$flag) {
                Mage::getModel('ey_protocol/customer')
                    ->setTimestamps(now())
    ...
    protected function _getProductList($term)
    {
        ...
        $key = 'protocol-product-name-' . $term . $storeId;
        ...
        if(!$products = $cache->load($key)){
            ...
            if($products->getSize()){
                ...
                $cache->save(urlencode($serializeReports), $key, array("protocol_name_cache"), 60*60*24);
    ...
    public function downloadAction()
    {
        ...
        $customer = Mage::getSingleton('customer/session')->getCustomer();
        Mage::dispatchEvent('protocol_download_after', array(
                'customer' => $customer,
                'id' => $data['id']
            )
        );

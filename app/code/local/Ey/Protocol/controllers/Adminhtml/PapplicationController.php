...
class Ey_Protocol_Adminhtml_PapplicationController extends Mage_Adminhtml_Controller_Action
{
    public function indexAction()
    {
        $block = $this->getLayout()
            ->createBlock('ey_protocol/adminhtml_application');
    ...
    {
        $application = Mage::getModel('ey_protocol/application');
        if ($applicationId = $this->getRequest()->getParam('id', false)) {
            ...
            if (!$application->getId()) {
                ...
                return $this->_redirect(
                    'adminhtml/papplication/index'
                );
            } elseif($application->getIsSent() == 0){
                $sent = Mage::helper('ey_protocol/email')->send(
                    array(
            ...
            foreach($postData as $id => $data){
                if($id == 'image_name'){
                    if(isset($data['delete']) && $data['delete'] == 1){
                        $postData[$id] = '';
                        $media_path  = Mage::getBaseDir('media') . '/protocol/' . DS;
        ...
        $applicationEditBlock = $this->getLayout()->createBlock(
            'ey_protocol/adminhtml_application_edit'
        );
    ...
    {
        $article = Mage::getModel('ey_protocol/application');
        ...
        switch ($actionName) {
            ...
            default:
                ...
                $isAllowed = $adminSession
                    ->isAllowed('ey_protocol/application');
                break;
        }
    ...
    protected function _getProductList($term)
    {
        ...
        $key = 'protocol-product-sku-' . $term . $storeId;
        ...
        if(!$products = $cache->load($key)){
            ...
            if($products->getSize()){
                ...
                $cache->save(urlencode($serializeReports), $key, array("protocol_sku_cache"), 60*60*24);

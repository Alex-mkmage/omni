    ...
    public function quickSaveAction()
    {
        ...
        if (isset($fileSession[$registerKey])) {
            foreach ($fileSession[$registerKey] as $fileDesc) {
                ...
                $filesCatIds = $data['fd_file']['category_id'];
                $filesCatText = Mage::helper('downloads')->getFileCategoriesText($filesCatIds);
                $file->setCategoryId(implode(',',$filesCatIds));
            ...
            try {
                ...
                if (isset($fileSession[$registerKey])) {
                    ...
                    foreach ($fileSession[$registerKey] as $fileDesc) {
                        ...
                        $filesCatIds = $data['general']['category_id'];
                        $filesCatText = Mage::helper('downloads')->getFileCategoriesText($filesCatIds);
                        $file->setCategoryId(implode(',',$filesCatIds));
                        $file->setCategoryText($filesCatText);
                        $file->setName(substr($fileDesc['name'], 0, strrpos($fileDesc['name'], '.')));
    ...
    public function massCategoryAction()
    {
        ...
        if (!is_array($ids)) {
            ...
            try {
                $model = Mage::getSingleton('downloads/files');
                foreach ($ids as $id) {
                    $filesCatIds = $this->getRequest()->getParam('category_id');
                    $filesCatText = Mage::helper('downloads')->getFileCategoriesText($filesCatIds);
                    $model->load($id)
                        ->setCategoryId(implode(',',$filesCatIds))
                        ->setCategoryText($filesCatText)
                        ->save();
                }
                $this->_getSession()->addSuccess($this->__('Total of %d record(s) were successfully updated', count($ids)));
            } catch (Exception $e) {
    ...
    public function runImportAction()
    {
        ...
        if ($current < $total) {
            ...
            for ($i = $current; $i < ($current + $limit); $i++) {
                ...
                $filesCatIds = $data['category_id'];
                $filesCatText = Mage::helper('downloads')->getFileCategoriesText($filesCatIds);
                $fileModel->setCategoryId(implode(',',$filesCatIds));
                $fileModel->setCategoryText($filesCatText);
                $fileModel->save();

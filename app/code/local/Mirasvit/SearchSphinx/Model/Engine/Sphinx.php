...
class Mirasvit_SearchSphinx_Model_Engine_Sphinx extends Mirasvit_SearchIndex_Model_Engine
{
    ...
    protected function prepareFullQuery($query, $result)
    {
        ...
        if (count(Mage::helper('core/string')->splitWords($query, true, $wordsLimit)) > 1) {
            $baseQuery = $this->prepareFullTerm($query);
            $baseQuery .= ' | ';
        }
    ...
    /**
     * Builds full search term query (for multiple words in seacrh term)
     *
     * @param  string $query
     *
     * @return string
    ...
    private function prepareFullTerm($query)
    {
        $terms = explode(' ', $query);
        $arWords = array();

        foreach ($terms as $term) {
            if (count($terms) >= count(explode(' ', $query))-1 ) {
                $arWords[] = trim($this->escapeSphinxQL(implode(' ', $terms)));
                array_pop($terms);
            } else {
                continue;
            }
        ...
        $isWildcardEnabled = Mage::getSingleton('searchsphinx/config')->isWildcardEnabled();
        $tmpQuery = array();
        
        foreach ($arWords as $searchTerm) {
            if ($isWildcardEnabled) {
                $tmpQuery[] = '(("*'. $searchTerm .'*") | ("'. $searchTerm .'"))';
            } else {
                $tmpQuery[] = '(("*'. $searchTerm .'*"))';
            }
        }

        return implode(' | ', $tmpQuery);
    }
